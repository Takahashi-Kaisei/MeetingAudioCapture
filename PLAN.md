...
- Coreターゲットは `MeetingAudioCaptureCore`、Appターゲットは `MeetingAudioCaptureApp` に統一する。
...
## K: M4A/WAV/MP3出力オプション

目的:
- ASR向けに非圧縮または高品質な音声形式を選べるようにする。

実装方針:
- `M4A` と `WAV` は `AVAudioFile` で直接保存する。
- `MP3` は一時M4Aを作成し、`ffmpeg -i input.m4a -ar 44100 -ab 128k output.mp3` 相当で変換する。
- `FLAC` はmacOS/CoreAudio/AVAudioFileでの安定対応を技術検証してから追加する。
- `AudioOutputFormat` enumを追加し、拡張子と `AVAudioFile` settings、必要な後処理を切り替える。
- 初期値はMVPと同じ `M4A`。
- 出力形式はメニューで選択し、`UserDefaults` に保存する。

検証:
- M4A/WAV/MP3それぞれのファイル生成テスト。
- M4A/WAVは `AVAudioFile` で読み返せること。
- MP3は一時M4Aからの変換経路と、`ffmpeg` 未導入時のエラーをテストする。
...
- MP3は `ffmpeg` の concat で結合する。
- 結合成功後は `_merged` ファイルだけを残し、元の分割ファイルは削除する。
- 結合失敗時は元ファイルを残し、エラーを表示する。

検証:
- 小さいM4A/WAVファイルを複数生成して結合するテスト。
- MP3は外部結合器を注入して結合経路をテストする。
- 結合後ファイルが再生/読み込み可能であること。
- 結合成功時は元ファイルが削除され、結合失敗時は元ファイルが残ること。
...
7. Add selectable audio output formats
